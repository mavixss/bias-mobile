...
import Profile from "./Profile";
  ...
  const InvestorsInfo = ({route}) => {
    const { user_id } = route.params;
    const { notif_id } = route.params;
    const { buss_id } = route.params;
    const { invst_id } = route.params;
...
const fetchData = () => {
  ...
  setTimeout(() => {
    const newData = [...displaydata, { key: Date.now().toString() }];
...
//to display each requested business

useEffect(() => {
    Axios.post("http://192.168.8.103:19001/InvstmentFeeds",{
      userID:user_id,
      notifID:notif_id,
      bussID:buss_id,
      invstID:invst_id
      ...
      // .then((res) => setData(res.data.results[0]))
      .then((res) => setdisplaydata(res.data.results))
		  ...
		  renderItem={({ item }) => (
			<TouchableOpacity style={styles.post}>
              ...
              <View style={styles.header}>
                ...
                />
                <View>
            <Text style={styles.name}>{user_id} {notif_id} {buss_id} {invst_id} {item.user_id} {item.user_fname} {item.user_lname}
			{/* {item.name} */}
			</Text>
